...
class DenseNet(nn.Module):
    ...
    block_config    :   list(int)
                        3 or 4 ints,  how many layers in each pooling block
    num_init_features   :   int
                            the number of filters to learn in the first convolution layer
    bn_size :   int
                multiplicative factor for number of bottle neck layers (i.e. ``bn_size`` * ``k``
    ...
    small_inputs    :   bool
                        set to True if images are ``32x32`` (e.g. CIFAR). Otherwise assumes images
                        are larger.
    efficient   :   bool
    ...
    def __init__(self, input_shape, growth_rate=12, block_config=(16, 16, 16), compression=0.5,
                 ...
                 num_classes=10, small_inputs=True, efficient=False, exporter=None):
        ...
        if small_inputs:
            self.features = nn.Sequential(OrderedDict([
                ('conv0', nn.Conv2d(C, num_init_features, kernel_size=3, stride=1, padding=1,
                                    bias=False)),
            ]))
        else:
            self.features = nn.Sequential(OrderedDict([
                ('conv0', nn.Conv2d(C, num_init_features, kernel_size=7, stride=2, padding=3,
                                    bias=False)),
            ]))
